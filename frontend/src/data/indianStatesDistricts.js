...
export const getAllStatesAndUTs = () => {
  return Object.keys(indianStatesAndUTs).sort();
};

// Get districts for a specific state/UT
export const getDistricts = (stateOrUT) => {
  return indianStatesAndUTs[stateOrUT]?.districts || [];
};

// Check if it's a state or UT
export const getType = (stateOrUT) => {
  return indianStatesAndUTs[stateOrUT]?.type || "";
};
